  ...
  <div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-10">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('inventarios.index') }}">Transportes</a></li>
...
@section('content')
  <div class="row mb-3">
    <div class="col-12">
      <a class="btn btn-default btn-sm" href="{{ route('transportes.index') }}"><i class="fa fa-reply" aria-hidden="true"></i> Volver</a>
      @if(Auth::user()->tipo <= 2)
        <a class="btn btn-default btn-sm" href="{{ route('transportes.edit', ['transporte' => $transporte->id]) }}"><i class="fa fa-pencil" aria-hidden="true"></i> Editar</a>
        ...
        <div class="tab-content">
          <div id="tab-1" class="tab-pane active">
            <div class="panel-body">
              <div class="mb-3">
                <button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#addModal"><i class="fa fa-plus" aria-hidden="true"></i> Agregar a Contrato</button>
              </div>
                ...
                <tbody class="text-center">
                  @foreach($transporte->contratos()->get() as $d)
                    <tr>
                      ...
                      @if(Auth::user()->tipo <= 2)
                        <td>
                          <button class="btn btn-danger btn-xs" data-contrato="{{ $d->id }}" data-toggle="modal" data-target="#delContratoModal"><i class="fa fa-times" aria-hidden="true"></i></button>
                        </td>
                      @endif
                    </tr>
                  @endforeach
          ...
          <div id="tab-2" class="tab-pane">
            <div class="panel-body">
              <div class="mb-3">
                <a class="btn btn-primary btn-sm" href="{{ route('consumos.create', ['transporte' => $transporte->id]) }}"><i class="fa fa-plus" aria-hidden="true"></i> Nuevo Consumo</a>
              </div>
                ...
                <tbody class="text-center">
                  @foreach($transporte->consumos()->get() as $d)
                    <tr>
                      ...
                      <td>{{ $d->valor() }}</td>
                      <td>
                        <a class="btn btn-success btn-sm" href="{{ route('consumos.show', ['consumo' => $d->id]) }}"><i class="fa fa-search"></i></a>
                      </td>
                    </tr>
                  @endforeach
                </tbody>
  ...
  @if(Auth::user()->tipo <= 2)
    <div id="addModal" class="modal inmodal fade" tabindex="-1" role="dialog" aria-labelledby="addModalLabel">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <form action="{{ route('transportes.storeContratos', ['transporte' => $transporte->id]) }}" method="POST">
            {{ csrf_field() }}
    ...
    <div id="delModal" class="modal inmodal fade" tabindex="-1" role="dialog" aria-labelledby="delModalLabel">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <form action="{{ route('transportes.destroy', ['transporte' => $transporte->id]) }}" method="POST">
            {{ method_field('DELETE') }}
            {{ csrf_field() }}
