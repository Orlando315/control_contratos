    ...
    <div class="col-md-6">
      <div class="ibox">
        ...
        <div class="ibox-content">
          <form action="{{ route('admin.archivo.carpeta.update', ['carpeta' => $carpeta->id]) }}" method="POST">
            @method('PATCH')
            @csrf

            <div class="form-group{{ $errors->has('nombre') ? ' has-error' : '' }}">
              <label for="nombre">Nombre: *</label>
              <input id="nombre" class="form-control" type="text" name="nombre" maxlength="50" value="{{ old('nombre', $carpeta->nombre) }}" placeholder="Nombre de la carpeta">
            </div>
              ...
              <div class="form-group{{ $errors->has('usuarios') ? ' has-error' : '' }}">
                ...
                <select id="usuarios" class="form-control" name="usuarios[]" multiple style="width: 100%">
                  ...
                  @foreach($users as $user)
                    <option value="{{ $user->id }}"{{ $carpeta->archivoUsers->contains($user->id) ? ' selected' : '' }}>{{ $user->nombre() }}</option>
                  @endforeach
                </select>
                
                <span class="form-text text-muted">Usuarios que tendán acceso a la carpeta.</span>
                @if($carpeta->isSubfolder())
            ...
            <div class="text-right">
              <a class="btn btn-default btn-sm" href="{{ route('archivo.carpeta.show', ['carpeta' => $carpeta->id]) }}"><i class="fa fa-reply"></i> Atras</a>
