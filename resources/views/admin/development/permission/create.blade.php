    ...
    <div class="col-md-6">
      <div class="ibox">
        <div class="ibox-title">
          <h5>Agregar permission</h5>
        </div>
        <div class="ibox-content">
          <form action="{{ route('admin.development.permission.store') }}" method="POST">
            @csrf
            ...
            <div class="form-group{{ $errors->has('modulo') ? ' has-error' : '' }}">
              ...
              <select id="modulo" class="form-control" name="modulo" required>
                ...
                @foreach($modulos as $modulo)
                  <option value="{{ $modulo->id }}"{{ old('modulo') == $modulo->id ? ' selected' : '' }}>{{ $modulo->name() }} ({{ $modulo->description ?? 'N/A' }})</option>
                @endforeach
              </select>
            </div>
            ...
            <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
              ...
              <small class="form-text text-muted">modulo-accion</small>
            </div>
            ...
            <div class="text-right">
              <a class="btn btn-default btn-sm" href="{{ route('admin.development.permission.index') }}"><i class="fa fa-reply"></i> Atras</a>
