        ...
        <div class="ibox-content">
          ...
          <form action="{{ route('admin.inventario.v2.update', ['inventario' => $inventario->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div id="bodegas-box" class="mb-2">
              @forelse($inventario->bodegas as $index => $inventarioBodega)
                <div class="row">
                  <div class="col-md-4">
                    <div class="form-group">
                      ...
                      <select id="bodega-{{ $index }}" class="form-control" name="bodegas[]">
                        ...
                        @foreach($bodegas as $bodega)
                          <option value="{{ $bodega->id }}"{{ $inventarioBodega->id == $bodega->id ? ' selected' : '' }}>{{ $bodega->nombre }}</option>
                        @endforeach
                      </select>
                    </div>
                  </div>
...
@section('script')
  ...
  <script type="text/javascript">
    ...
    const UBICACIONES = @json($inventarioUbicaciones);
    ...
    $(document).ready(function(){
      $('#unidad, select[id^="bodega"], select[id^="ubicacion"], #categorias').select2({
        theme: 'bootstrap4',
        placeholder: 'Seleccione...',
      });

      $('#bodegas-box').on('change', 'select[id^="bodega"]', searchUbicaciones);
      $('select[id^="bodega"]').change();
    ...
    function searchUbicaciones() {
      ...
      .done(function (response) {
        ...
        $.each(response, function (k, ubicacion) {
          let selected = UBICACIONES.includes(ubicacion.id);
          $(inputUbiacion).append(`<option value="${ubicacion.id}"${selected ? ' selected' : ''}>${ubicacion.nombre}</option>`);
        });
      })
      .fail(function () {
        $(inputUbiacion).empty().prop('disabled', true);
