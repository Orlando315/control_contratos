  ...
  <div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-10">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('admin.inventario.v2.show', ['inventario' =>  $egreso->inventario_id]) }}">Ingreso de Stock</a></li>
    ...
    <div class="col-md-10">
      <div class="ibox">
        ...
        <div class="ibox-content">
          <form action="{{ route('admin.inventario.egreso.update', ['egreso' => $egreso->id]) }}" method="POST" enctype="multipart/form-data">
            @method('PATCH')
            @csrf
              ...
              <div class="col-md-4">
                <div class="form-group{{ $errors->has('tipo') ? ' has-error' : '' }}">
                  ...
                  <select id="tipo" class="custom-select" name="tipo">
                    <option value="usuario"{{ old('tipo', $egreso->tipo()) == 'usuario' ? ' selected' : '' }}>Usuario</option>
                    <option value="cliente"{{ old('tipo', $egreso->tipo()) == 'cliente' ? ' selected' : '' }}>Cliente</option>
                  </select>
                </div>
              </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="form-group{{ $errors->has('usuario') ? ' has-error' : '' }}">
                  ...
                  <select id="usuario" class="form-control" name="usuario" style="width: 100%">
                    ...
                    @foreach($usuarios as $usuario)
                      <option value="{{ $usuario->id }}"{{ old('usuario', $egreso->user_id) == $usuario->id ? ' selected' : '' }}>
                ...
                <div class="form-group{{ $errors->has('cliente') ? ' has-error' : '' }}">
                  ...
                  <select id="cliente" class="form-control" name="cliente" disabled style="width: 100%">
                    ...
                    @foreach($clientes as $cliente)
                      <option value="{{ $cliente->id }}"{{ old('cliente', $egreso->cliente_id) == $cliente->id ? ' selected' : '' }}>
              ...
              <div class="col-md-4">
                <div class="form-group{{ $errors->has('cantidad') ? ' has-error' : '' }}">
                  ...
                  <input id="cantidad" class="form-control" type="number" name="cantidad" min="0" max="9999" value="{{ old('cantidad', $egreso->cantidad) }}" placeholder="Cantidad" required>
                </div>
              </div>
              <div class="col-md-4">
                <div class="form-group{{ $errors->has('costo') ? ' has-error' : '' }}">
                  <label for="costo">Costo:</label>
                  <input id="costo" class="form-control" type="number" name="costo" min="0" max="999999999" step="0.01" value="{{ old('costo', $egreso->costo) }}" placeholder="Costo">
                </div>
              </div>
            </div>
                ...
                <div class="form-group{{ $errors->has('contrato') ? ' has-error' : '' }}">
                  ...
                  <select id="contrato" class="form-control" name="contrato">
                    ...
                    @foreach($contratos as $contrato)
                      <option value="{{ $contrato->id }}"{{ old('contrato', $egreso->contrato_id) == $contrato->id ? ' selected' : '' }}>
                        {{ $contrato->nombre }}
                      </option>
                    @endforeach
                  </select>
                ...
                <div class="form-group{{ $errors->has('faena') ? ' has-error' : '' }}">
                  ...
                  <select id="faena" class="form-control" name="faena">
                    ...
                    @foreach($faenas as $faena)
                      <option value="{{ $faena->id }}"{{ old('faena', $egreso->faena_id) == $faena->id ? ' selected' : '' }}>
                        {{ $faena->nombre }}
                      </option>
                    @endforeach
                  </select>
                ...
                <div class="form-group{{ $errors->has('centro_costo') ? ' has-error' : '' }}">
                  ...
                  <select id="centro_costo" class="form-control" name="centro_costo">
                    ...
                    @foreach($centrosCostos as $centroCosto)
                      <option value="{{ $centroCosto->id }}"{{ old('centro_costo', $egreso->centro_costo_id) == $centroCosto->id ? ' selected' : '' }}>
                        {{ $centroCosto->nombre }}
                      </option>
                    @endforeach
                  </select>
            ...
            <div class="row">
              <div class="col-md-8">
                <div class="form-group{{ $errors->has('descripcion') ? ' has-error' : '' }}">
                  ...
                  <textarea id="descripcion" class="form-control" name="descripcion" cols="30" rows="3" maxlength="250">{{ old('descripcion', $egreso->descripcion) }}</textarea>
                </div>
              </div>
              <div class="col-md-4">
                <div class="form-group">
                  <div class="text-center">
                    <a id="foto-link" href="#" type="button">
                      <img id="foto-placeholder" class="img-fluid border" src="{{ $egreso->foto ? $egreso->foto_url : asset('images/default.jpg') }}" alt="foto" style="max-height:120px;margin: 0 auto;">
                    </a>
                  </div>
            ...
            <div class="text-right">
              <a class="btn btn-default btn-sm" href="{{ route('admin.inventario.egreso.show', ['egreso' => $egreso->id]) }}"><i class="fa fa-reply"></i> Atras</a>
